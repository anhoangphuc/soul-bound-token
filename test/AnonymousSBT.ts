...
import { encryptAMessage, decryptMessage } from './utils';

context('Anonymous', async () => {
    ...
    it(`Mint a new SBT`, async () => {
        ...
        const commitment = ethers.utils.keccak256(utils.toUtf8Bytes(`${secret}-${age}`));
    ...
    it(`Verify account1`, async () => {
        ...
        const commitment = ethers.utils.keccak256(utils.toUtf8Bytes(`${secret}-${age}`));
        expect((commitment === ageInfo.commitment) && (age >= 18));
    });
})